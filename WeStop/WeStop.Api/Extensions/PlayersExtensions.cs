...
namespace WeStop.Api.Extensions
{
    public static class PlayersExtensions
    {
        ...
        {
            ICollection<Answer> answersOfOthersPlayers = GetAnswersOfOtherPlayersInRound(players, roundId, playerId);
            ...
            foreach (var themeAnswer in answersOfOthersPlayers)
            {
                ...
                {
                    ThemeAnswers themeAnswers = new ThemeAnswers(themeAnswer.Theme, themeAnswer.Value);
        ...
        private static ICollection<Answer> GetAnswersOfOtherPlayersInRound(IEnumerable<Player> players, Guid roundId, Guid playerId)
        {
        ...
        {
            ICollection<Answer> answersOfOthersPlayers = GetAnswersForThemeOfOtherPlayersInRound(players, roundId, playerId, theme);
        ...
        private static ICollection<Answer> GetAnswersForThemeOfOtherPlayersInRound(IEnumerable<Player> players, Guid roundId, Guid playerId, string theme)
        {
            return players.Where(p => p.Id != playerId)
